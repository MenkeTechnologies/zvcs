...
#    `alien`, `topic`, `link-to-file`, `refs/heads/dangling`, `wt-held`,
#    `outermost`, `refs/notes/review`, `ita-new.txt`, `hf-side`, `pc-side` --
#    so adding the shape without adding its names draws the fixture and then
#    asks it about `main` and `feature`, which resolve to nothing. Every shape
#    added here comes with the names that make it reachable.
#
...
#
# The other direction fails quietly, and has three times now: a shape fixture.rs
# builds that no generated grammar draws is unreachable, and nothing says so.
# `Shape::ALL` gained 14 shapes and then 7 more before this map caught up.
#
# Being in this map is NOT the same as being reachable, and the difference is
# what the third round cost. The map was brought up to date with all 38 shapes
# and the warning went quiet -- while ten of them (`hooked` and the nine that
# landed with it) were named by no grammar at all and so were still drawn by
# nothing. Membership here only makes a shape *spellable*; a grammar has to
# spell it. So both directions are checked below, against `Shape::name`'s arms:
# a fixture shape this map does not name, and a fixture shape no grammar names.
my %SHAPE = (
    'linear'           => 'Shape::Linear',
...
my @fixture_shapes = fixture_shape_names();

my @unmapped = grep { !$SHAPE{$_} } @fixture_shapes;
printf STDERR "warning: %d shape(s) fixture.rs builds cannot be spelled by any grammar (not in %%SHAPE): %s\n",
...
# Which shape names the JSON actually spells. Counted only for grammars that
# survive to `@entries`: a grammar with no fuzzable surface is dropped whole, so
# the shapes it lists are drawn by nothing.
my %drawn;
...
for my $f (@files) {
    ...
    my @shapes;
    for my $s (@sh) {
        if (my $v = $SHAPE{$s}) { push @shapes, $v; $drawn{$s} = 1 }
        else { push @bad, "$f: unknown shape \"$s\""; }
    }
    # A grammar that lists no shapes still draws one, so `linear` is reached by
    # the default as surely as by being spelled.
    unless (@shapes) { @shapes = ('Shape::Linear'); $drawn{'linear'} = 1 }
...
# The check `%SHAPE` membership cannot make. A shape can be spellable and still
# be drawn by nothing, which is the state ten of them were in.
my @undrawn = grep { $SHAPE{$_} && !$drawn{$_} } @fixture_shapes;
printf STDERR "warning: %d shape(s) fixture.rs builds are drawn by no grammar: %s\n",
    scalar(@undrawn), join(' ', @undrawn) if @undrawn;
