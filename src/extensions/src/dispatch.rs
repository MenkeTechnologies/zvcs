...
/// git.c:499-500, `run_builtin()`'s `setup_work_tree()` for a `NEED_WORK_TREE`
/// builtin. Also used by callers that run such a builtin in-process as a
/// `run_command()` child (`bisect` spawning `checkout`), since the child passes
/// the same gate.
///
...
fn z_usage(sub: &str) -> Option<&'static str> {
    Some(match sub {
        ...
        "zpin" => "usage: git zpin [<path>...|list] [--json] — freeze repos from daemon autonomy (no autobump/reconcile until unpinned)",
        ...
        "zsince" => "usage: git zsince <duration|snapshot> [--kind K] [--repo R] [--json] — everything across the tree since a time (90s/45m/2d/1h30m) or snapshot",
        "zcontend" => "usage: git zcontend [--json] — live agent-vs-agent contention: claims, per-repo job backlog, contested repos",
        "zwaitfor" => "usage: git zwaitfor <clean|idle|synced|<repo> <sha>> [--timeout <secs>] — block until a tree-wide state holds",
        "zgraph" => "usage: git zgraph [--json] — fleet topology: dup groups (same origin, multiple local checkouts)",
        ...
        "zrepos" => "usage: git zrepos [<pattern>...] [--json] — list indexed repos; patterns filter by case-insensitive substring",
        ...
        "zjobs" => "usage: git zjobs [-n <count>] [--json] — list recent ledger jobs (newest first)",
        ...
        "zwho" => "usage: git zwho [--json] — list active claims (who is working what)",
        "zstatus" => "usage: git zstatus [--all] [--json] — cached working-tree status of indexed repos",
        ...
        "zheads" => "usage: git zheads [selectors] [--json] — HEAD branch/id (+dirty) of each indexed repo, in parallel",
        "zdirty" => "usage: git zdirty [selectors] [--json] — list indexed repos with uncommitted tracked changes, in parallel",
        "zbranches" => "usage: git zbranches [selectors] [--json] — local branches of each indexed repo, in parallel",
        "ztags" => "usage: git ztags [selectors] [--json] — tag count of each indexed repo, in parallel",
        "zremotes" => "usage: git zremotes [selectors] [--json] — remotes and fetch URLs of each indexed repo",
        "zsize" => "usage: git zsize [selectors] [--json] — on-disk .git size of each indexed repo (largest first)",
        "zage" => "usage: git zage [selectors] [--json] — how long ago each indexed repo's HEAD commit was made",
        ...
        "zgrep" => "usage: git zgrep [selectors] [-i] <pattern> [--json] — parallel regex search of tracked content across indexed repos",
        "zahead" => "usage: git zahead [selectors] [--json] — indexed repos with commits not yet on their upstream",
        "zbehind" => "usage: git zbehind [selectors] [--json] — indexed repos whose upstream is ahead of them",
        "zunpushed" => "usage: git zunpushed [selectors] [--json] — per-repo unpushed commits (id + summary); the detailed zahead",
        "zunpulled" => "usage: git zunpulled [selectors] [--json] — per-repo commits the upstream has that local lacks; the detailed zbehind",
        "zauthors" => "usage: git zauthors [selectors] [--json] — commit counts by author across indexed repos, ranked",
        "zhot" => "usage: git zhot [selectors] [<days>] [--json] — indexed repos ranked by commits in the last <days> (default 30)",
        "zconflicts" => "usage: git zconflicts [selectors] [--json] — indexed repos mid-merge/rebase/cherry-pick/revert/bisect or with conflicts",
        ...
        "zstale" => "usage: git zstale [selectors] [<days>] [--json] — indexed repos whose HEAD is older than <days> (default 90)",
        "zlast" => "usage: git zlast [selectors] [--json] — indexed repos ordered by HEAD commit time, newest first",
        "zbig" => "usage: git zbig [selectors] [<n>] [--json] — largest tracked files across indexed repos (top <n>, default 20)",
        "zfiles" => "usage: git zfiles [selectors] [--json] — tracked file count of each indexed repo (largest first)",
        "zcommits" => "usage: git zcommits [selectors] [--json] — HEAD-history commit count of each indexed repo (deepest first)",
        "zpristine" => "usage: git zpristine [selectors] [--json] — indexed repos that are clean, attached, and in sync (nothing to do)",
        "zdivergent" => "usage: git zdivergent [selectors] [--json] — indexed repos both ahead of and behind their upstream",
        "zorphans" => "usage: git zorphans [selectors] [--json] — indexed repos with no remote configured",
        "zsessions" => "usage: git zsessions [--json] — active sessions ranked by repos held",
        "zidle" => "usage: git zidle [selectors] — indexed repos with no active claim (free to pick up)",
        "zdashboard" => "usage: git zdashboard [--json] — instant one-screen health summary from the status cache + ledger",
