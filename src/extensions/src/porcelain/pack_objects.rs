...
fn path_walk_filter_compatible(spec: &[u8]) -> Result<(), u64> {
    if let Some(rest) = spec.strip_prefix(b"tree:".as_slice()) {
        ...
        let depth = super::list_objects_filter::git_parse_ulong(rest).unwrap_or(0);
        if depth != 0 {
            return Err(depth);
        }
        return Ok(());
    ...
    if let Some(body) = spec.strip_prefix(b"combine:".as_slice()) {
        ...
        for segment in body.split(|&c| c == b'+') {
            ...
            path_walk_filter_compatible(&super::list_objects_filter::url_percent_decode(segment))?;
        }
    }
    Ok(())
}
...
/// `gently_parse_list_objects_filter` with pack-objects' `allow_auto_filter = 0`,
/// reduced to its verdict: `Err(msg)` carries the exact text git puts after
/// `fatal: `. The grammar itself lives in [`super::list_objects_filter`].
pub(super) fn gently_parse_filter(arg: &[u8]) -> Result<(), String> {
    super::list_objects_filter::gently_parse_list_objects_filter(arg, false).map(drop)
}
