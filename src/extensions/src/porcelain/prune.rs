...
//!
//! A shallow repository is served too: `cmd_prune()` finishes with
//! `prune_shallow()`, which regenerates `.git/shallow` from the same reachability
//! marks the object sweep used, keeping the boundary commits the walk reached and
//! unlinking the file when none are left. See [`prune_shallow`].
//!
...
/// boundary commits the reachability walk actually reached.
///
/// The file is regenerated by `write_shallow_commits_1(&sb, 0, NULL, SEEN_ONLY)`,
/// whose `write_one_shallow()` drops any entry whose commit is not marked `SEEN`
/// — so an entry survives exactly when the object sweep above called it
/// reachable. A run that keeps nothing unlinks the file rather than leaving an
/// empty one, which is what stops a repository from looking shallow with no
/// boundary in it.
///
/// `PRUNE_SHOW_ONLY` (`prune -n`) takes the early return: the same walk is done
/// for its `Removing <oid> from .git/shallow` lines and nothing is written.
/// Those lines go to **stdout**, since `write_one_shallow()` uses `printf`.
fn prune_shallow(
    repo: &gix::Repository,
    reachable: &HashSet<ObjectId>,
    dry_run: bool,
) -> Result<()> {
    let path = repo.common_dir().join("shallow");
    ...
    for line in text.lines() {
        ...
        if reachable.contains(&oid) {
            ...
            println!("Removing {oid} from .git/shallow");
        }
    }
    ...
    if kept.is_empty() {
        ...
        // Through a `.lock` sibling, as `hold_lock_file_for_update()` does, so a
        // reader never sees a half-written boundary list.
        let lock = path.with_extension("lock");
        fs::write(&lock, kept.as_bytes())?;
        fs::rename(&lock, &path)?;
    }
    Ok(())
}
