...
const FLAG_OPTS: &[&str] = &[
    ...
    "--dirstat-by-file",
    "--cumulative",
    // The `--color-moved` / `--word-diff` family, whose renderers `cmd_log()` shares
    // (see [`super::log`]): recognised here so they reach the deprecation notice,
    // and plumbed there so `--i-still-use-this` renders them.
    "--word-diff",
    "--color-words",
    "--color-moved",
    "--no-color-moved",
