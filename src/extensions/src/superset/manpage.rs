...
pub const DOCS: &[Doc] = &[
    ...
    Doc {
        ...
        desc: &[
            ...
            "The advice <cmd> is a shell command run via `sh -c`. It sees the intercepted command through the environment: INTERCEPT_NAME (the subcommand), INTERCEPT_ARGS, and INTERCEPT_CMD (the full `git ...` line); after-advice also gets INTERCEPT_STATUS, INTERCEPT_MS, and INTERCEPT_US. An around advice runs `eval \"$INTERCEPT_CMD\"` to proceed with the original (that child is not re-intercepted) \\(em the `eval` is required, because INTERCEPT_CMD is a whole command line and the quoted-without-eval form makes the shell look for a single program by that name.",
