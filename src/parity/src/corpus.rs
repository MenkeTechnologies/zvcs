...
mod config_cmd;
mod config_reads;
mod conflict_classes;
mod dates_namespaces;
mod diff_family;
mod exit_codes;
mod fetch_clone;
mod fixture_gaps;
...
pub fn cases() -> Vec<Case> {
    ...
    conflict_classes::cases(&mut c);
    dates_namespaces::cases(&mut c);
    ...
    history_simplification::cases(&mut c);
    history_query::cases(&mut c);
    hooks_identity::cases(&mut c);
    ignore_engine::cases(&mut c);
    ...
    merge_file::cases(&mut c);
    misc_commands::cases(&mut c);
    object_listing::cases(&mut c);
    object_pack::cases(&mut c);
    pathspec_stdin::cases(&mut c);
    plumbing_objects::cases(&mut c);
    plumbing_refs::cases(&mut c);
    am_deep::cases(&mut c);
    commit_message::cases(&mut c);
    pull_family::cases(&mut c);
    stash_internals::cases(&mut c);
    tag_family::cases(&mut c);
    clone_options::cases(&mut c);
    discovery_env::cases(&mut c);
    index_format::cases(&mut c);
    merge_ort::cases(&mut c);
    object_format::cases(&mut c);
    archive_formats::cases(&mut c);
    config_resolution::cases(&mut c);
    grep_engine::cases(&mut c);
    rerere_engine::cases(&mut c);
    traversal_order::cases(&mut c);
    apply_patch::cases(&mut c);
    diff_algorithms::cases(&mut c);
    eol_conversion::cases(&mut c);
    status_formats::cases(&mut c);
    submodule_deep::cases(&mut c);
    bisect_replay::cases(&mut c);
    external_tools::cases(&mut c);
    patch_equivalence::cases(&mut c);
    rewrite_tools::cases(&mut c);
    worktree_lifecycle::cases(&mut c);
    integrity_gc::cases(&mut c);
    naming_ancestry::cases(&mut c);
    ref_storage::cases(&mut c);
    text_plumbing::cases(&mut c);
    wire_protocol::cases(&mut c);
    blame_lines::cases(&mut c);
    interchange::cases(&mut c);
    notes_family::cases(&mut c);
    sparse_family::cases(&mut c);
